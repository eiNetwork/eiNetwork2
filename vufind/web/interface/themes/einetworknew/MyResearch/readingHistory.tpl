	...
	<div id="main-content" class="col-xs-9 col-md-9">
		...
		{if $transList}		
			<div class="pull-right">
					Sort by&nbsp<select name="sort" onchange="document.location.href = '/MyResearch/ReadingHistory?page={$page|escape}&accountSort='+this.options[this.selectedIndex].value;">
...
-->										<input class="btn btn-warning" type="button" onclick='return deleteAllAction()' value="Delete All">
							...
							{/if}
							<td class="myAccountCell">
								<input class="btn btn-warning" type="button" value="Delete" onclick="deleteOne('{$record.rsh|escape:"url"}')" />
							</td>
						</tr>
					{/foreach}
				</tbody>
