	    ...
	    <div class="checkout">
			...
			<table class="checkouts">
					...
					{foreach from=$transList item=record name="recordLoop"}
						...
						<tr>
							<td class="title" style="width:85%" colspan="2">
								{if $record.id}
								...
								<td style="width:15%" rowspan="3">
									{if $user->disableCoverArt != 1}
										...
										<a href="{$url}/Record/{$record.id|escape:"url"}" id="descriptionTrigger{$record.id|escape:"url"}">
										    <img src="{$coverUrl}/bookcover.php?id={$record.id}&amp;isn={$record.isbn|@formatISBN}&amp;size=small&amp;upc={$record.upc}&amp;category={$record.format_category.0|escape:"url"}" class="bookcover" alt="{translate text='Cover Image'}"/>
										</a>
									    {/if}
									{/if}
								</td>
						...
						{if $record.renewMessage}
							<tr>
								<td colspan="3" class="{if $record.renewResult == true}renew_pass{else}renew_fail{/if}">
			...
			<table class="checkouts">
				...
				<tr>
					<td class="title" style="width:100%" colspan="3">
				...
				<tr>
					<td style="width:15%" rowspan="2">
						<img class="bookcover" src="{$record.imageUrl}">
					</td>
					...
					<td style="text-align:right" rowspan="2">
						...
						{/if}
						<input class="button" type="button" value="Download" onclick="DownloadCheckedoutOverdrive({$record.recordId}{if $record.formatSelected},'{$record.selectedFormat.format}'{/if})"/><img class="qtip-od-download" src="/images/help_icon.png" /></br>
